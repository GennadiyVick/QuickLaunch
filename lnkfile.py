...
# -- RoganovGV - editing the script fix unicode issues , edit read_stringData function
#    and add new functions: getUnicodeString, get_run_file and getLinkInfo
...
class lnk_file(object):
    def __init__(self, fhandle=None, indata=None, debug=False, getrunfileEncoding='windows-1251'):
        self.define_static()
        self.getrunfileEncoding = getrunfileEncoding
        if fhandle:
    ...
    def define_common(self):
        try:
            ...
            if self.debug: print('Exception define_common: %s' % e)
            #if self.debug:

    def get_run_file(self):
        pos = 0x18
        if self.linkFlag['HasTargetIDList']:
            pos = self.targets['size'] + self.lnk_header['header_size'] + 2

        last_pos = pos
        pos += 0x04
        length = struct.unpack('I', self.indata[last_pos:pos])[0]
        pos += 0x0C
        lbpos = struct.unpack('I', self.indata[pos:pos+0x04])[0]
        pos = last_pos + lbpos
        size= (length + last_pos) - pos - 0x02
        return self.indata[pos:pos+size].decode(self.getrunfileEncoding)
    ...
    def getUnicodeString(self, data):
        '''fixing incorrect unicode encoding in windows'''
        temp = list(data)
        i = 0
        l = len(temp)
        while i < l:
            if temp[i+1] == 92: #'\'
                temp[i] = 0 #some times non-English is not 0, but must be
                if temp[i+4] != 0: #after the ch. '\' if there is a non-English text is 0 correct it on temp[i+4]
                    temp[i+2] = temp[i+4]
            if temp[i+1] == 0x2e: #'.'
                if temp[i+2] == 0:
                    temp[i] = 0
        ...
        #this fix is not quite correct but working for me
        return bytes(temp).decode('utf-16be')
    ...
    def read_stringData(self, index, u_mult):
        ...
        #next line corrected by RoganovGV 1 instead of 2
        data = self.indata[index + 1: index + 1 + string_size]
        if self.linkFlag['IsUnicode']:
            string = self.getUnicodeString(data)
        else:
            string = self.clean_line(data.replace(b'\x00', b''))
...
def getLinkInfo(fn):
    ...
    fs = os.stat(fn)
    if fs.st_size < 4:
        print('file size is zero or read access denied')
        return {}


    with open(fn, 'rb') as file:
        lnk = lnk_file(fhandle=file)
        data = lnk.data.copy()
        for key in data:
            if type(data[key]) == 'str':
                data[key] = data[key].replace('\\\\','\\')
        data['runfile'] = lnk.get_run_file()
        if len(data['runfile']) < 2:
            data['runfile'] = ''
            if 'relativePath' in data:
                rp = data['relativePath']
                if 'workingDirectory' in data:
                    wd = data['workingDirectory']
                    data['runfile'] = os.path.join(wd, os.path.basename(rp))
                else:
                    data['runfile'] = rp
        data['iconindex'] = lnk.lnk_header['icon_index']
        '''
            data['description'], data['relativePath'], data['workingDirectory']
            data['commandLineArguments'], data['iconLocation']
        '''
        return data
